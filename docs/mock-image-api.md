...
- `http://127.0.0.1:8787/drop-image-tool`：模拟中转丢弃 `image_generation` 工具却保留 `tool_choice`，返回 `Tool choice 'required' must be specified with 'tools' parameter.`；应用应报出该错误并附上「当前 API 在转发时丢弃了 image_generation 工具」的提示。
